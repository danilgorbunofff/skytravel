...
// Each instance keeps its own abort controller and bootstrap cache, so admin
// usage never mutates the public search page state (and vice versa).
function makeStore() {
  return createSearchStoreInstance();
}

// Separate instances: admin mutations never leak into the public search page.
export const useSearchStore = makeStore();
export const useAdminSearchStore = makeStore();
