...
Push to `main` triggers `deploy.yml` → SSH into production server (`167.233.47.103`) → `npm ci` → Prisma migrate → build → PM2 restart. Manual deploy: `bash scripts/deploy-remote.sh` (requires `ssh-key-new.key` in repo root).
