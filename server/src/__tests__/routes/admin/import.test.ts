...
describe("Admin import — invalid provider", () => {
  ...
  it("POST /api/admin/providers/:id/import with unknown provider returns 404",
    async () => {
  ...
  it("POST /api/admin/providers/:id/import with empty ids array returns 400",
    async () => {
      const res = await supertest(app)
        .post("/api/admin/providers/alexandria/import")
        .send({ ids: [] });
  ...
  it("GET /api/admin/providers/nonexistent/regions returns 404", async () => {
    const res = await supertest(app).get(
      "/api/admin/providers/nonexistent/regions",
    );
    assert.equal(res.status, 404);
    assert.equal(res.body.error.code, "NOT_FOUND");
  });
...
describe("Admin import — provider dispatch", { skip: skipWithoutDb }, () => {
  ...
  it("POST /api/admin/providers/alexandria/import dispatches to the provider",
    async () => {
      const res = await supertest(app)
        .post("/api/admin/providers/alexandria/import")
